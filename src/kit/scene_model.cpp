...
Ptr<Texture> SceneModel::getTexture(unsigned int textureIndex) const
{
...
void SceneModel::addTexture(std::string const & filename, std::string const & type, unsigned int uvIndex)
{
	Ptr<Texture> texture = App::get()->getResourceCache()->getTextureFromFile(filename);
...
void SceneModel::setColor(Vector3f const & _emitColor, Vector4f const & _diffuseColor)
{
...
void SceneModel::render(Matrix44f const & projectionTransform, Matrix44f const & localToCameraTransform, std::vector<Vector3f> const & lightPositions, std::vector<Vector3f> const & lightColors) const
{
...
void SceneModel::updateShader()
{
	...
	shader = App::get()->getResourceCache()->getShader(name, code);
	sorted = false;
