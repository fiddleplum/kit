...
namespace kit
{
	namespace scene
	{
		...
		void CameraInternal::setAspectRatio (float newAspectRatio)
		{
			aspectRatio = newAspectRatio;
			projectionNeedsUpdate = true;
		}

		void CameraInternal::setNear (float newNear)
		{
			near = newNear;
			projectionNeedsUpdate = true;
		}

		void CameraInternal::setFar (float newFar)
		{
			far = newFar;
			projectionNeedsUpdate = true;
		}

		void CameraInternal::setPerspective (float newFov)
		{
			fov = newFov;
			perspective = true;
			projectionNeedsUpdate = true;
		...
		void CameraInternal::setOrthogonal (float newSize)
		{
			size = newSize;
			perspective = false;
			projectionNeedsUpdate = true;
		...
		void CameraInternal::setPosition (Vector3f position)
		{
			EntityInternal::setPosition(position);
			viewNeedsUpdate = true;
		}

		void CameraInternal::setOrientation (Quaternionf orientation)
		{
			EntityInternal::setOrientation(orientation);
			viewNeedsUpdate = true;
		}

		Vector2f CameraInternal::getNdcPosition (Vector3f positionInWorld) const
		{
			if(projectionNeedsUpdate)
			{
				const_cast<CameraInternal *>(this)->updateProjection();
			}
			if(viewNeedsUpdate)
			{
				const_cast<CameraInternal *>(this)->updateView();
			}
			return (projection * view).transform(positionInWorld, 1).shrink<2>();
		}

		Ray3f CameraInternal::getRay (Vector2f ndcPosition) const
		{
			if(projectionNeedsUpdate)
			{
				const_cast<CameraInternal *>(this)->updateProjection();
		...
		Matrix44f const & CameraInternal::getProjection () const
		{
			if(projectionNeedsUpdate)
			{
				const_cast<CameraInternal *>(this)->updateProjection();
		...
		Matrix44f const & CameraInternal::getView () const
		{
			if(viewNeedsUpdate)
			{
				const_cast<CameraInternal *>(this)->updateView();
		...
		void CameraInternal::updateProjection ()
		{
			float scale;
			if(perspective)
			{
		...
		void CameraInternal::updateView ()
		{
