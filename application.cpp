...
void openWindow ()
{
  ...
  wc.lpszClassName = L"toolbox";
  wc.cbWndExtra = 4;
  RegisterClass(&wc);

  // Create the window. hWnd is set in the WM_CREATE message, called from CreateWindow.
  CreateWindow(L"toolbox", L"default title", WS_OVERLAPPEDWINDOW | WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);
  ...
  {
    MessageBox(NULL, L"Could not initialize OpenGL Extensions.", 0, 0);
    exit(-1);
  }
