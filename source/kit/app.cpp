...
void App::create(bool usingAudio)
{
	if(!app.isValid())
	{
		app.create(usingAudio);
	}
}
...
App::App(bool usingAudio)
{
	//firstMouseMoveEvent = true;

	// Start SDL.
	if(SDL_InitSubSystem(SDL_INIT_VIDEO | SDL_INIT_JOYSTICK | SDL_INIT_GAMECONTROLLER | SDL_INIT_TIMER) == -1)
	{
		throw std::runtime_error(std::string("Could not initialize SDL Subsystems: ") + SDL_GetError() + ". ");
	}
	...
	switch(sdlEvent.type)
	{
		...
		case SDL_MOUSEBUTTONUP:
			...
			if(window.isValid())
			{
				...
				event.cursorPosition.set(sdlEvent.button.x, sdlEvent.button.y);
				handleEvent(event);
			}
			break;
		...
		case SDL_MOUSEWHEEL:
			...
			if(window.isValid())
			{
				...
				event.cursorPosition.set(sdlEvent.wheel.x, sdlEvent.wheel.y);
				handleEvent(event);
			}
			break;
